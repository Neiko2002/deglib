...
static std::vector<tsl::robin_set<size_t>> get_ground_truth(const uint32_t *ground_truth, const size_t ground_truth_size, const size_t k) {
...
static float test_approx(const hnswlib::HierarchicalNSW<float> &appr_alg, const float* query_repository, const std::vector<tsl::robin_set<size_t>> &ground_truth, const size_t query_dims, const size_t k) {
  ...
  for (int i = 0; i < ground_truth.size(); i++) {
    ...
    while (result_queue.empty() == false) {
      if (gt.find(result_queue.top().second) != gt.end()) 
        correct++;
      result_queue.pop();
    }
  }
...
static void test_vs_recall(hnswlib::HierarchicalNSW<float> &appr_alg, const float* query_repository, const std::vector<tsl::robin_set<size_t>> &ground_truth, const size_t query_dims, const size_t k) {
  ...
  for (int i = max(30, k); i < 100; i += 10)
  {
      efs.push_back(i);
  }
  for (int i = max(100, k); i < 500; i += 40)
  {
      efs.push_back(i);
  }
  for (size_t ef : efs)
...
int main()
{
    ...
    auto ground_truth = ivecs_read(path_groundtruth, top_k, query_size);
    auto query_features = fvecs_read(path_query, vecdim, query_size);
    auto base_features = fvecs_read(path_basedata, vecdim, base_size);
