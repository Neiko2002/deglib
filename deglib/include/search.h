 ...
 * k: the amount of similar nodes which should be returned
 */
struct YahooSearchStaticFunctor
{
    const deglib::StaticGraph& graph;
    const std::vector<deglib::ObjectDistance>& entry_nodes;
    const float* query;
    const deglib::L2Space& l2space;
    const float eps;
    const int k;

    template <size_t EdgeCount, size_t Dimensions>
    deglib::ResultSet operator()()
    {
        const auto dist_func = l2space.get_dist_func();
        const auto dist_func_param = l2space.get_dist_func_param();
                                    ...
                                    const deglib::L2Space& l2space, const float eps, const int k)
{
    ...
    return deglib::invoke_chosing_edge_count_and_feature_vector_size(
        YahooSearchStaticFunctor{graph, entry_nodes, query, l2space, eps, k}, graph.edge_count(), graph.dims());
}
